...
import { randomDirection } from "./util/sphere";
...
// While fading out a streak decelerates by up to this fraction of its speed and
// its visible body retracts toward the head by up to this fraction of its length.
const FADE_SLOWDOWN = 0.8;
const FADE_RETRACTION = 0.7;
...
interface Streak {
  ...
  // Fixed per-trail-point xyz jitter, picked once at spawn so the trail keeps a
  // stable shape instead of re-scattering (and shimmering) every frame.
...
function createStreak(): Streak {
  return {
    ...
    radial: new Vector3(),
    drift: 0,
    scatter: new Float32Array(TRAIL_POINTS * 3),
  };
}
...
export class ShootingStars {
  ...
  private readonly tailPoint = new Vector3();
  ...
  private spawn(streak: Streak): void {
    ...
    randomDirection(streak.origin).multiplyScalar(radius);
    streak.radial.copy(streak.origin).normalize();
    randomTangent(streak.radial, streak.direction, this.tailPoint);
    ...
    for (let i = 0; i < streak.scatter.length; i++) {
      streak.scatter[i] = Math.random() - 0.5;
    }
    streak.progress = 0;
    streak.active = true;
  }
  ...
  private advance(streak: Streak, head: Line, trail: Points): void {
    const fadePhase =
      streak.progress > streak.length
        ...
        : 0;
    streak.progress += streak.speed * (1 - fadePhase * FADE_SLOWDOWN);
  ...
  private updateTrailGeometry(
    ...
    const visibleLength = streak.length * (1 - fadePhase * FADE_RETRACTION);
    ...
    for (let p = 0; p < TRAIL_POINTS; p++) {
      ...
      const point = this.tailPoint
        .copy(streak.origin)
        .addScaledVector(streak.direction, trailProgress)
        .add(this.driftOffset);
      // Scatter grows toward the tail (u=0 is the head); each point's direction
      // is fixed at spawn (streak.scatter) so the trail holds its shape.
      const scatter = (1 - (1 - u) * (1 - u)) * TRAIL_SCATTER;
      point.x += streak.scatter[p * 3] * scatter;
      point.y += streak.scatter[p * 3 + 1] * scatter;
      point.z += streak.scatter[p * 3 + 2] * scatter;
