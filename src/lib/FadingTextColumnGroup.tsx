...
export default function FadingTextColumnGroup(props: Props) {
  ...
  const tRef = useRef(0);
  // Mirrors `opacity` so the per-frame loop can skip setState when it hasn't moved.
  ...
  useFrame((state) => {
    ...
    // Only push opacity into React state when it actually moves: scale.get() is
    // constant while the spring is settled, so re-rendering the whole text column
    // every frame (opacity feeds the FlatClickText props) would be wasted work.
    const currentOpacity = scale.get();
    if (Math.abs(currentOpacity - opacityRef.current) > 0.001) {
  ...
  const columnOffset = (textDistance * (props.textArray.length - 1)) / 2;
